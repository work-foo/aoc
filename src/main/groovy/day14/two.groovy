...
acc =
resource.getText().replaceAll(~/[^-\d]/, ' ').trim().split()*.toInteger().collate(4).findResults { int px, int py, int vx, int vy ->
...
.entrySet().inject(1) {accum, val -> accum * val.getCount()}
